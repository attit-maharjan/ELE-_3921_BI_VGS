...
from users.models import User, Student, Address
from django.conf import settings


class Command(BaseCommand):
    help = "Import students from a CSV file and create user accounts."

    def handle(self, *args, **kwargs):
        csv_file = settings.BASE_DIR / 'data' / 'csv' / 'student.csv'
            ...
            for row in reader:
                ...
                if created:
                    ...
                    Student.objects.create(
                        user=user,
                        enrollment_date=parse_date("2016-01-05")
                    )
                    self.stdout.write(self.style.SUCCESS(f"Student created: {email}"))
